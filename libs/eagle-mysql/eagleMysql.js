...
module.exports = (function() {
     ...
     * var insertParams = {                             <br/>
     *     table  : 'T_TEST_USER',                      <br/>
     *     keys   : ['USERNAME', 'PASSWORD'],           <br/>
     *     values : [                                   <br/>
                ['myname1', 'mupassword1'],             <br/>
                ['myname2', 'mupassword2']              <br/>
            ]                                           <br/>
     * };                                               <br/>
     * eagleMysql.insertMulti(insertParams, {           <br/>
     *     success : function (data) {                  <br/>
     *         console.log('success');                  <br/>
     *     },                                           <br/>
     *     error : function (err) {                     <br/>
     ...
     * @method insertMulti
     */
    o.insertMulti = function (params, callback) {
        var values = [];
        for (var i=0,len= params.values.length; i<len; i++) {
            values.push('(\'' + params.values[i].join("\',\'") + "\')");
        }
        
        if (o.debug) {
            console.log('INSERT INTO ' + params.table + '(' + params.keys.join(',') +  ')' + ' VALUES' + values.join(",") + ';');
        }
        this.client.query('INSERT INTO ' + params.table + '(' + params.keys.join(',') +  ')' + ' VALUES' + values.join(",") + ';',
            function (err, results, fields) {
                o._doCallback(err, results, fields, callback);
            }
        );
